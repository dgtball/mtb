...
async def get_upcoming_payments(request: Request, year: int = None, month: int = None):
    ...
    try:
        ...
        dividends = [{"ticker": row[0], "date": row[1], "amount": row[2], "type": "dividend"} for row in await c.fetchall()]
        ...
        """, (str(year), f"{month:02d}"))
        coupons = [{"ticker": row[0], "date": row[1], "amount": row[2], "type": "coupon"} for row in await c.fetchall()]
